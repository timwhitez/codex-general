...
DEFAULT_GITHUB_REPO = "timwhitez/codex-general"
BINARY_TARGETS = (
...
def install_from_workflow_artifacts(
    ...
    artifacts = select_target_artifacts(workflow_id, workflow_repo, components)
    download_artifacts(workflow_id, workflow_repo, artifacts_dir, artifacts)
...
def select_target_artifacts(
    ...
    artifacts_by_name = {
        ...
        for artifact in list_workflow_artifacts(workflow_id, workflow_repo)
    }
...
def list_workflow_artifacts(
    ...
    stdout = subprocess.check_output(
        [
            "gh",
            "api",
            f"repos/{workflow_repo}/actions/runs/{workflow_id}/artifacts",
            "--paginate",
            "--jq",
...
def tarball_name_for_package(package: str, version: str) -> str:
    ...
    if package in CODEX_PLATFORM_PACKAGES:
        platform = CODEX_PLATFORM_PACKAGES[package]["npm_tag"]
...
def main() -> int:
    ...
    try:
        if native_component_sets:
            ...
            workflow_repo = repo_from_workflow_url(workflow_url)
            print(
                f"Using native artifacts from {workflow_url} ({workflow_repo})",
                flush=True,
            )

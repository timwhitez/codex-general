...
The TypeScript SDK wraps the `codex-general` CLI from `codex-general`. It spawns the CLI and exchanges JSONL events over stdin/stdout.

## Installation

```bash
npm install codex-general-sdk
```
...
```typescript
import { Codex } from "codex-general-sdk";

...
fn personal_access_token_exec_command(server: &MockServer, home: &TempDir) -> Command {
    let bin = codex_utils_cargo_bin::cargo_bin("codex-general").unwrap();
...
// Use this for new `codex-general exec` subprocess tests in this file. These commands
...
/// Verify that `codex-general exec --profile ...` preserves the active user config
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    let bin2 = codex_utils_cargo_bin::cargo_bin("codex-general").unwrap();

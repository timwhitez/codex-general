...
struct Cli {
    ...
    /// spawning `codex-general app-server` as a child process.
...
#[derive(Subcommand)]
enum CliCommand {
    /// Start `codex-general app-server` on a websocket endpoint in the background.
    ///
    /// Logs are written to:
    ///   `/tmp/codex-app-server-test-client/`
    Serve {
        /// WebSocket listen URL passed to `codex-general app-server --listen`.
...
pub async fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let codex_bin = codex_bin.unwrap_or_else(|| PathBuf::from("codex-general"));
...
fn serve(codex_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    ...
    println!("started codex-general app-server");
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdin, .. } => {
                if let Some(stdin) = stdin.as_mut() {
                    ...
                    stdin
                        .flush()
                        .context("failed to flush payload to codex-general app-server")?;
                    return Ok(());
                }
                bail!("codex-general app-server stdin closed")
            }
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdout, .. } => {
                ...
                let bytes = stdout
                    ...
                    .context("failed to read from codex-general app-server")?;
                if bytes == 0 {
                    bail!("codex-general app-server closed stdout");
                }
                Ok(response_line)
            }
...
impl Drop for CodexClient {
    fn drop(&mut self) {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            println!("[codex-general app-server exited: {status}]");
            return;
        }

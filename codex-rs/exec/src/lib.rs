...
enum StdinPromptBehavior {
    ...
    /// `codex-general exec` behavior for `codex-general exec` with piped input.
    RequiredIfPiped,
    /// Always treat stdin as the prompt, used for the explicit `codex-general exec -`
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    if let Err(err) = set_default_originator("codex_exec".to_string()) {
        tracing::warn!(
            ?err,
            "Failed to set codex-general exec originator override {err:?}"
        );
    }

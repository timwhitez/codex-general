...
#[command(
    version,
    override_usage = "codex-general exec [OPTIONS] [PROMPT]\n       codex-general exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
...
struct ResumeArgsRaw {
    ...
    // so "codex-general resume --last <prompt>" treats the positional as a prompt, not a session id.

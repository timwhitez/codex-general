...
fn profile_v2_for_subcommand<'a>(
    ...
    match subcommand {
        ...
        _ => anyhow::bail!(
            "--profile only applies to runtime commands and `codex-general mcp`: `codex-general`, `codex-general exec`, `codex-general review`, `codex-general resume`, `codex-general fork`, `codex-general mcp`, and `codex-general debug prompt-input`."
        ),
    }
}
...
async fn load_exec_server_remote_auth_provider(
    ...
    let auth = load_exec_server_remote_auth(
        config,
        "remote exec-server registration requires ChatGPT authentication; run `codex-general login` first",
    )
    .await?;

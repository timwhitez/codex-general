...
const CODEX_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT: &str =
    "You are Codex, a general-purpose agent running in the Codex CLI, a terminal-based assistant.";
    ...
    assert!(
        message.contains(&format!("session {conversation_id} is archived"))
            && message.contains(&format!(
                "codex-general unarchive {conversation_id}` to unarchive it first"
            )),

...
`codex-general app-server` is the interface Codex uses to power rich interfaces such as the [Codex VS Code extension](https://marketplace.visualstudio.com/items?itemName=openai.chatgpt).
...
Similar to [MCP](https://modelcontextprotocol.io/), `codex-general app-server` supports bidirectional communication using JSON-RPC 2.0 messages (with the `"jsonrpc":"2.0"` header omitted on the wire).
...
The unix socket transport is intended for local app-server control-plane clients. `codex-general app-server proxy`
...
Currently, you can dump a TypeScript version of the schema using `codex-general app-server generate-ts`, or a JSON Schema bundle via `codex-general app-server generate-json-schema`. Each output is specific to the version of Codex you used to run the command, so the generated artifacts are guaranteed to match that version.

```
codex-general app-server generate-ts --out DIR
codex-general app-server generate-json-schema --out DIR
```
...
Applications building on top of `codex-general app-server` should identify themselves via the `clientInfo` parameter.
...
- **Personal access token (`personalAccessToken`)**: Codex uses a ChatGPT-backed personal access token loaded outside the app-server login RPCs, such as with `codex-general login --with-access-token` or `CODEX_ACCESS_TOKEN`.
...
`codex-general app-server` schema generation defaults to the stable API surface (experimental fields and methods filtered out). Pass `--experimental` to include experimental methods/fields in generated TypeScript or JSON schema:
...
codex-general app-server generate-ts --out DIR --experimental
codex-general app-server generate-json-schema --out DIR --experimental
```

...
- Server binary: `codex-general mcp-server` (or `codex-mcp-server`)
...
```bash
codex-general mcp-server | your_mcp_client
```
...
Use the separate `codex-general mcp` subcommand to manage configured MCP server launchers in `config.toml`.

...
- The fork release workflow is `.github/workflows/codex-general-release.yml`; keep upstream `.github/workflows/rust-release.yml` from running on `rust-v*` tag pushes in this fork. The successful full release method was validated by `codex-general-release #16` for tag `rust-v2026.5.28`.
- Date-based releases use tags like `rust-v2026.5.28`, and the tag version must match `codex-rs/Cargo.toml`. After committing the version bump, push the branch and move the annotated release tag to the release commit: `git push timwhitez HEAD:codex-general-pr HEAD:main`, then `git tag -f -a rust-vYYYY.M.D -m "Release YYYY.M.D"` and `git push --force timwhitez rust-vYYYY.M.D`. Do not move the release tag for later docs-only commits.
- Use the `release-gh` Cargo profile with `timeout-minutes: 360`, `CARGO_INCREMENTAL=0`, `CARGO_PROFILE_RELEASE_LTO=off`, `CARGO_PROFILE_RELEASE_CODEGEN_UNITS=16`, `CARGO_PROFILE_RELEASE_GH_LTO=off`, and `CARGO_PROFILE_RELEASE_GH_CODEGEN_UNITS=16`.
- The validated GitHub-hosted matrix is: `ubuntu-latest` / `x86_64-unknown-linux-musl` / `cargo_jobs: "2"`; `windows-latest` / `x86_64-pc-windows-msvc` / `cargo_jobs: "2"`; `macos-15-intel` / `x86_64-apple-darwin` / `cargo_jobs: "1"`; `macos-15` / `aarch64-apple-darwin` / `cargo_jobs: "1"`. Keep `fail-fast: false` so one slow platform does not cancel completed artifacts from the others.
...
- On GitHub-hosted macOS runners, keep macOS release Cargo build pressure low. `macos-15-intel` for `x86_64-apple-darwin` is slow but succeeds with `CARGO_BUILD_JOBS=1` and the 360-minute timeout. Do not switch this fork to `macos-15-large` unless the account billing/spending limit has been confirmed, because that paid larger runner can fail before starting.
- The `codex-general-release #19` run for `rust-v2026.6.9` confirmed that the `macos-15-intel` / `x86_64-apple-darwin` build can be the last job and take about three hours while the other platforms have already succeeded. Do not cancel, rerun, or redesign the matrix solely because this job appears slow while it is still making progress and is within the 360-minute timeout.
- Install the pinned `codex-rs/rust-toolchain.toml` toolchain explicitly before any release `cargo` invocation, and keep that installation logged/uploaded. The `codex-general-release #17` mac-x64 failure for `rust-v2026.6.1` surfaced publicly only as `info: syncing channel updates for 1.95.0-x86_64-apple-darwin` inside `Build Codex General`; avoid lazy rustup sync in the long build step by preserving the retrying toolchain install step and using `cargo +$RUSTUP_TOOLCHAIN`.
- Keep `CARGO_NET_GIT_FETCH_WITH_CLI: "true"` in the release workflow. During the `rust-v2026.6.9` merge/release, local `just test -p codex-cli` failed before running tests because the external Chromium `libyuv` submodule fetch (`https://chromium.googlesource.com/libyuv/libyuv`) returned HTTP 502, even with this setting. Treat that exact failure as an external dependency-host outage to retry or report, not as a test failure or a reason to churn code.
- Build `codex-general` on every platform; on Windows also build `codex-command-runner` and `codex-windows-sandbox-setup`; on Linux build bundled `bwrap` first and export `CODEX_BWRAP_SHA256` from the produced binary. Keep release build logs uploaded with `if: always()` and emit a `::error` tail when Cargo fails. GitHub public annotations may otherwise show only `exit code 101`, which is not enough to diagnose release failures.
- When monitoring a release without authenticated GitHub credentials, the REST API can hit the 60-request unauthenticated rate limit and return a JSON payload where `.jobs` is absent. Do not interpret `jq` errors or null fields from that payload as workflow state; inspect HTTP headers/body for rate limiting and fall back to public HTML pages such as the run page, `graph_partial`, matrix `graph/matrix/...expanded=true`, and `releases/expanded_assets/<tag>`.
...
- If the extracted `bin/codex-general --version` check is run from a package under `/tmp`, it may print a warning about refusing to create PATH helper aliases under a temporary dir. That warning is expected in this verification context; the check is valid if the command exits successfully and reports the expected `codex-cli <version>`.
- For annotated release tags, `git ls-remote <remote> refs/tags/<tag>` shows the tag object SHA, not the commit SHA. Use `git show-ref --dereference --tags <tag>` or equivalent to verify the tag dereferences to the intended release commit.
